...
func (c *client) doTimeoutRequest(timer *time.Timer, req *http.Request) (*http.Response, error) {
	...
	case r := <-done:
		return r.resp, r.err
	case <-timer.C:
		return nil, errors.New("timeout on reading data from Hpool API")
	}
}

// do prepare and process HTTP request to Hpool API
func (c *client) do(method string, ressource string, payload map[string]string, authNeeded bool) (response []byte, err error) {
	...
	if authNeeded {
		if len(c.authToken) == 0 {
			err = errors.New("You need to set Auth Token to call this method")
			return
		}
